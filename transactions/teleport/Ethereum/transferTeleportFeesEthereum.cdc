...
transaction(target: Address) {
  ...
  let teleportAdminRef: auth(TeleportCustodyEthereum.AdminEntitlement) &TeleportCustodyEthereum.TeleportAdmin

  prepare(teleportAdmin: auth(BorrowValue) &Account) {
    self.teleportAdminRef = teleportAdmin.storage.borrow<auth(TeleportCustodyEthereum.AdminEntitlement) &TeleportCustodyEthereum.TeleportAdmin>(from: TeleportCustodyEthereum.TeleportAdminStoragePath)
