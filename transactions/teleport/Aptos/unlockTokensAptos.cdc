...
transaction(amount: UFix64, target: Address, from: String, hash: String) {
    ...
    let teleportControlRef: auth(TeleportCustodyAptos.AdminEntitlement) &{TeleportCustodyAptos.TeleportControl}

    // The Receiver reference of the user
    let receiverRef: &{FungibleToken.Receiver}

    prepare(teleportAdmin: auth(BorrowValue) &Account) {
        self.teleportControlRef = teleportAdmin.storage.borrow<auth(TeleportCustodyAptos.AdminEntitlement) &{TeleportCustodyAptos.TeleportControl}>(from: TeleportCustodyAptos.TeleportAdminStoragePath)
