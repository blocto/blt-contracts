...
transaction(admin: Address, amount: UFix64, target: String) {
    ...
    let teleportUserRef: &{TeleportCustodyBSC.TeleportUser}

    // The Vault resource that holds the tokens that are being transferred
    let sentVault: @{FungibleToken.Vault}

    prepare(signer: auth(BorrowValue) &Account) {
        self.teleportUserRef = getAccount(admin).capabilities.borrow<&{TeleportCustodyBSC.TeleportUser}>(TeleportCustodyBSC.TeleportAdminTeleportUserPath)
